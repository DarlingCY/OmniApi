...
| 模式 | 命令 | 说明 |
| --- | --- | --- |
| 桌面 | `omni-api.exe`（默认 `--mode desktop`） | 常驻系统托盘，默认监听 `127.0.0.1:47831`，托盘菜单“打开配置”带管理 Token 打开浏览器 |
| 服务 | `omni-api --mode server --host 0.0.0.0 --port 47831 --data-dir /data` | 无 GUI，适合容器；非 Windows 构建不链接任何桌面依赖 |

参数与环境变量：`--mode`/`OMNI_MODE`、`--host`/`OMNI_HOST`、`--port`/`OMNI_PORT`、`--data-dir`/`OMNI_DATA_DIR`、`OMNI_MASTER_KEY`、`OMNI_STATIC_DIR`、`OMNI_WEB_URL`。
...
该命令先把控制台构建到 `internal/webui/dist`，再产出内嵌前端的 `omni-api.exe`（约 12 MB）。Linux 服务镜像：

```powershell
docker compose build
$env:OMNI_MASTER_KEY = "<32 字节以上随机串>"
docker compose up -d
```

## 配置顺序

1. 在“访问设置”中设置管理 Token 和外部调用使用的代理 Token。
...
4. 在“模型中心”查看最终对外暴露的模型。暴露名称相同的上游模型自动组成一个负载均衡组，按供应商顺序、再按供应商内模型顺序轮询。
...
管理接口在管理 Token 为空时开放，用于首次引导；一旦设置就强制校验。代理接口始终要求代理 Token。绑定到 `0.0.0.0` 且任一 Token 为空时，启动日志会打印告警。

## 外部调用

请求使用 `Authorization: Bearer <代理 Token>`，请求体中的 `model` 填模型中心里的暴露名称。`GET /v1/models` 同样需要代理 Token，返回去重并升序排列的暴露名称。
...
控制台侧栏和 Windows 托盘菜单均提供“运行日志”入口。页面每秒自动刷新，可暂停、按级别或关键词筛选，点击请求 ID 可查看同一次请求的各个阶段。

日志包含请求进入、请求解析、上游转发、失败重试、流式响应开始和最终结果；读取请求体、等待上游或流式传输超过 10 秒时，持续记录等待时长。网络错误会保留连接、TLS 或超时原因，流式读取失败会返回错误事件并记录失败。

运行日志只在内存保留本次启动的最近 1,000 条，重启后清空。已结束的请求仍保存到“请求监控”。日志不主动记录请求正文或鉴权头，已配置的密钥会脱敏，上游地址不包含用户名、密码和查询参数。管理接口 `GET /api/v1/runtime-logs` 使用与其他管理接口相同的鉴权规则。
...
Electron 已完全移除。`packages/**` 和 `tests/**` 是迁移前的 Fastify 版网关，仅作对照保留，运行时不再使用；`npm test` 与 `npm run typecheck` 只覆盖这部分历史代码。
